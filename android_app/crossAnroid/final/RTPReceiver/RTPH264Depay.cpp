...
bool RTPH264Depay::getStatus()
{
    return mEnable;
}

void RTPH264Depay::sendQueue(BufferQueue *queue)
{
    ...
    while (!queue->isEmpty()) {
        ...
        /* Implement funcs at here*/

        delete [] buffer;
    }
}

void RTPH264Depay::cleanQueue(BufferQueue *queue)
{
    unsigned char *buffer;
    struct h264Buffer *info;
    int out_idx = 0;
...
int RTPH264Depay::finishPackets(BufferQueue *queue)
{
    unsigned char *buffer;
    ...
    while (!queue->isEmpty()) {
        ...
        if (begin) {
            ...
            fuInfo->start = 1;
            if (!fuInfo->buffer) {
                cout << "fuInfo buffer is null!" << endl;
            }

            if (!info->buffer) {
                cout << "info buffer is null!" << endl;
            }
            ...
            if (end) {
                fuInfo->end = 1;
                outputQueue->pushTail((unsigned char *)fuInfo, fuInfo->size + sizeof(struct h264Buffer));
                flag = false;
            }
        } else {
            count += info->size;
...
void RTPH264Depay::setStreamMode(bool mode)
{
    byte_stream = mode;

}

bool RTPH264Depay::getStreamMode()
{
    return byte_stream;
}
...
RTPH264Depay::RTPH264Depay(void):fuQueue(NULL), singleQueue(NULL), outputQueue(NULL)
{
    ...
    singleQueue = new BufferQueue;
    outputQueue = new BufferQueue;
    if (!fuQueue || !singleQueue || !outputQueue) {
...
RTPH264Depay::~RTPH264Depay(void)
{
    ...
    while (!fuQueue->isEmpty()) {
        node = fuQueue->popHead();
        buffer = node->data;
        delete [] buffer;
    }
    ...
    switch (nal_unit_type) {
      ...
      case 28:
      {
        ...
        if (S) {
          ...
          fuQueue->pushTail((unsigned char *)info, info->size + sizeof(struct h264Buffer));
        } else {
          fu_header = payload[1];
      ...
      default:
      {
        ...
        singleQueue->pushTail((unsigned char *)info, info->size + sizeof(struct h264Buffer));
