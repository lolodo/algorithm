...
static struct reg_val_ops MAX9288_CAB888_1v1_init_cmd[] = {
	{MAX9288_ADDR,      {0x02, 0x00}, 0x0F,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x60, 0x00}, 0x23,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x65, 0x00}, 0x77,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x08, 0x00}, 0x28,             0x01, i2c_write},

	//{MAX9271_INIT_ADDR, {0x00, 0x00}, 0xA0,             0x01, i2c_write},
	//{MAX9288_ADDR,      {0x00, 0x00}, 0xA0,             0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x01, 0x00}, 0xD0,             0x01, i2c_write},

	/* address mapping */
	{MAX9288_ADDR,      {0x18, 0x00}, 0xA0,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x19, 0x00}, 0x80,             0x01, i2c_write},
	
	{MAX9271_INIT_ADDR, {0x04, 0x00}, 0x07,             0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x06, 0x00}, 0xA0,             0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x01, 0x00}, 0xD0,             0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x00, 0x00}, 0xA0,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x15, 0x00}, 0x03,             0x01, i2c_write},
	
	{MAX9271_INIT_ADDR, {0x07, 0x00}, 0x84,             0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x09, 0x00}, OV490_CH0_ADDR,   0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x0A, 0x00}, OV490_INIT_ADDR,  0x01, i2c_write},
	{MAX9271_INIT_ADDR, {0x04, 0x00}, 0x87,             0x01, i2c_write},
	{MAX9288_ADDR,      {0x09, 0x00}, 0xc0,             0x01, i2c_write},
		...
		u8 *reg, unsigned int reg_len, u8 *val)
{
	...
	*val = *(data + reg_len);
	kfree(data);
	data = NULL;
	max9288_info("read dev/reg/val/ret is %02x/%02x/%02x/%d",
...
static int max9288_get_lock_status(struct i2c_client *client, u8 *val)
{
	int ret = 0;
...
static int max9288_camera_init(struct i2c_client *client)
{
	...
	/*check video link*/
	for (read_cnt = 0; read_cnt < 3; ++read_cnt) {
		ret = max9288_get_link(client, &link_reg_val);
		if ((ret == 2) && (link_reg_val > 0u))
			break;

		if (read_cnt == 2) {
			max9288_err("no camera linked link_reg_val=%x",
				link_reg_val);
			return -ENODEV;
		}
	...
	for (index = 0; index < 4; ++index) {
		...
		if (ch_link_status[index] != 0u) {
			++link_cnt;
			max9288_info("channel %d linked", index);
		} else {
			max9288_info("channel %d not linked", index);
		}
	}
	...
	if (link_cnt == 1U) {
		ret = max9288_set_link_config(client, &link_config_reg_val);
		if (ret != 1)
			return -EIO;
	...
	max9288_info("%u camera linked", link_cnt);
#if 0
	...
	for (read_cnt = 0; read_cnt < 3; ++read_cnt) {
		...
		if (read_cnt == 2) {
			max9288_err("read lock status time out");
			return -EIO;
		}
