...
class Fbtool:
    ...
    def __find_usb_device(self, match_usb_func):
        while True:
            ...
            for port in ports:
                if match_usb_func(port.vid, port.pid):
                    time.sleep(0.1)
                    try:
                        self.__ser = serial.Serial(port.device, 115200)
                    except serial.SerialException as e:
                        logging.debug('%s, retry...' %(str(e)))
                    else:
                        logging.info('Got %s' %(port))
                        return
...
if __name__ == '__main__':
    ...
    if serial.VERSION < '3':
        logging.error('pySerial version(%s) is lower than 3.0, please upgrade!' %serial.VERSION)
