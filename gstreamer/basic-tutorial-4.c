/*************************************************************************
    > File Name: basic-tutorial-4.c
    > Author: Alex
    > Mail: fangyuan.liu@nio.com 
    > Created Time: 2017年12月03日 星期日 14时18分32秒
 ************************************************************************/
		...
		if (msg != NULL) {
			...
			if (data.playing){
				...
				if (!GST_CLOCK_TIME_IS_VALID(data.duration)) {
					if (!gst_element_query_position(data.playbin, GST_FORMAT_TIME, &data.duration)) {
						g_printerr("Could not query current duration.\n");
					}
				}
...
static void handle_message(Customdata *data, GstMessage *msg)
{
	...
	switch(GST_MESSAGE_TYPE(msg)) {
		...
		case GST_MESSAGE_EOS:
			g_print("End-Of-Stream reachecd.\n");
			data->terminate = TRUE;
			break;
