...
uint_t b_white[1080*1815];
uint_t b_black[1080*1815];
...
static void *app_function(void *userdata) {
    ...
    for (i = 0; i < 1080*1815; i++) { b_black[i] = 0; b_white[i] = 0xFFFFFFFF; }

    GST_DEBUG("Creating pipeline in CustomData at %p", data);
    g_data = data;

    pid_t tid = gettid();
    __android_log_print(ANDROID_LOG_ERROR, "screensharing",
                        "app_function tid:%d", tid);
    ...
    /* Build pipeline */
    //data->pipeline = gst_parse_launch(" tcpclientsrc host=10.111.160.35  port=8554  ! jpegdec ! videoconvert ! autovideosink", &error);

    /*data->pipeline = gst_parse_launch(
            "filesrc name=my_filesrc  ! decodebin !  videoconvert ! jpegenc !  tcpserversink port=8888 host=0.0.0.0",
            &error);
            */
    data->pipeline = gst_parse_launch(
            "appsrc name=my_appsrc  !  videoconvert ! jpegenc !  tcpserversink port=8554 host=0.0.0.0",
            //"appsrc name=my_appsrc  ! x264enc tune=\"zerolatency\" threads=1 ! mpegtsmux ! tcpserversink port=8554 host=0.0.0.0",
            //"appsrc name=my_appsrc  !  tcpserversink port=8554 host=0.0.0.0",
            &error);
    ...
    GstElement *appsrc;
    appsrc = gst_bin_get_by_name(GST_BIN(data->pipeline), "my_appsrc");
...
static void gst_native_init(JNIEnv *env, jobject thiz) {
    ...
    GST_DEBUG_CATEGORY_INIT(debug_category, "screensharing", 0, "Android tutorial 5");
    gst_debug_set_threshold_for_name("screensharing", GST_LEVEL_DEBUG);
    GST_DEBUG("Created CustomData at %p", data);
    data->app = (*env)->NewGlobalRef(env, thiz);
    GST_DEBUG("Created GlobalRef for app object at %p", data->app);
    pid_t tid = gettid();
    __android_log_print(ANDROID_LOG_ERROR, "screensharing",  "%s:%d thiz:%x tid:%d", __func__, __LINE__, thiz, tid);
...
static void gst_native_set_uri(JNIEnv *env, jobject thiz, jstring uri) {
    ...
    GstElement *filesrc;
    filesrc = gst_bin_get_by_name(GST_BIN(data->pipeline), "my_filesrc");
    g_object_set(filesrc, "location", char_uri, NULL);
...
void gst_native_push_buff(JNIEnv *env, jobject thiz, jobject obj_buff) {
    ...
    CustomData *data = GET_CUSTOM_DATA (env, thiz, custom_data_field_id);
    //if (!data || !data->pipeline) return;
    if (!g_data || !g_data->pipeline) return;
    data = g_data;
    ...
    /*
        ...
        jobject bar = (*env)->GetObjectField(env, thiz, fid);
        char *java_buffer = (char *) (*env)->GetDirectBufferAddress(env, bar);
        jlong size = (*env)->GetDirectBufferCapacity(env, bar);
        */

    char *java_buffer = (char*)(*env)->GetDirectBufferAddress(env, obj_buff);
    uint_t size = (*env)->GetDirectBufferCapacity(env, obj_buff);
    buffer = gst_buffer_new_wrapped_full( 0, (gpointer)(java_buffer), size, 0, size, NULL, NULL );
    /*
    static gboolean white = FALSE;
    uint_t size = 1080 * 1815 * 4;
    ...
    if (print_tid <= 1) {
        __android_log_print(ANDROID_LOG_ERROR, "screensharing",
                            "push buffer size:%u thiz:%x obj_buff:%x java_buffer:%x buffer:%x",
                            size, thiz, obj_buff, java_buffer, buffer);
    }
...
static JNINativeMethod native_methods[] = {
        ...
        {"nativePushBuff",        "(Ljava/nio/ByteBuffer;)V", (void *) gst_native_push_buff},
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if ((*vm)->GetEnv(vm, (void **) &env, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print(ANDROID_LOG_ERROR, "screensharing", "Could not retrieve JNIEnv");
        return 0;
    }
    jclass klass = (*env)->FindClass(env, "com/nextev/screensharing/activity/VirtualDisplayDemo");
    (*env)->RegisterNatives(env, klass, native_methods, G_N_ELEMENTS(native_methods));


    klass = (*env)->FindClass(env, "com/nextev/screensharing/activity/VirtualDisplayDemoImageReader");
    (*env)->RegisterNatives(env, klass, native_methods, G_N_ELEMENTS(native_methods));

    klass = (*env)->FindClass(env, "com/nextev/screensharing/activity/VirtualDisplayDemoNew");
    (*env)->RegisterNatives(env, klass, native_methods, G_N_ELEMENTS(native_methods));
